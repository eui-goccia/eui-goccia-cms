...
export async function up({ db, payload, req }: MigrateUpArgs): Promise<void> {
  ...
  await db.run(sql`CREATE TABLE \`events\` (
  	...
  	FOREIGN KEY (\`parent_event_id\`) REFERENCES \`events\`(\`id\`) ON UPDATE no action ON DELETE set null
  );
  `)
  await db.run(sql`CREATE INDEX \`events_cover_image_idx\` ON \`events\` (\`cover_image_id\`);`)
  await db.run(sql`CREATE INDEX \`events_parent_event_idx\` ON \`events\` (\`parent_event_id\`);`)
  ...
  await db.run(sql`CREATE TABLE \`_events_v\` (
  	...
  	\`version_parent_event_id\` text(36),
  	\`version_slug\` text,
  	...
  	FOREIGN KEY (\`version_parent_event_id\`) REFERENCES \`events\`(\`id\`) ON UPDATE no action ON DELETE set null
  );
  `)
  ...
  await db.run(sql`CREATE INDEX \`_events_v_version_version_parent_event_idx\` ON \`_events_v\` (\`version_parent_event_id\`);`)
